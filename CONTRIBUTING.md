...
Instead of using the `resp` object returned from client calls, the `err` object should be 
read instead. The err object from client calls will be a `*gitlab.ErrorResponse`, which already
contains the error code. There are several helper functions in the `api` package to make reading
and checking these errors easier, and they should always be used.
...
if api.Is400(err) {
  // do something if BadRequest
}
```

**Undesired:**

Don't cast inline with the code - add a new api helper if your status code isn't covered
```go
mirror, _, err := r.client.Projects.GetProjectPullMirrorDetails(project, gitlab.WithContext(ctx))
...
Don't use the httpResp object, the status code is already in the `err` return.
```go
mirror, httpResp, err := r.client.Projects.GetProjectPullMirrorDetails(project, gitlab.WithContext(ctx))
if httpResp.Code == 400 {
  // do something
}
```
