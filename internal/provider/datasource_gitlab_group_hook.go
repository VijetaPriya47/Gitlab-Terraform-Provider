...
var (
	_ datasource.DataSource              = &gitlabGroupHookDataSource{}
	_ datasource.DataSourceWithConfigure = &gitlabGroupHookDataSource{}
)

func init() {
	registerDataSource(NewGitlabGroupHookDataSource)
}

func NewGitlabGroupHookDataSource() datasource.DataSource {
	return &gitlabGroupHookDataSource{}
}

type gitlabGroupHookDataSource struct {
	client *gitlab.Client
}

type gitlabGroupHookDataSourceModel struct {
...
func (d *gitlabGroupHookDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_group_hook"
}

func (d *gitlabGroupHookDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: `The ` + "`gitlab_group_hook`" + ` data source allows to retrieve details about a hook in a group.

**Upstream API**: [GitLab REST API docs](https://docs.gitlab.com/api/group_webhooks/#get-a-group-hook)`,

		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				MarkdownDescription: "The ID of this data source. In the format `<group:hook_id>`.",
				Computed:            true,
			},
...
func (d *gitlabGroupHookDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *gitlabGroupHookDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data *gitlabGroupHookDataSourceModel
	...
	if err != nil {
		resp.Diagnostics.AddError("GitLab API error occurred", fmt.Sprintf("Unable to read group hook: %s", err.Error()))
		return
	}

	data.ID = types.StringValue(fmt.Sprintf("%s:%d", group, hookID))
	data.Group = types.StringValue(group)
	data.GroupID = types.Int64Value(int64(hook.GroupID))
	data.HookID = types.Int64Value(int64(hook.ID))
	data.URL = types.StringValue(hook.URL)
	data.Token = types.StringValue("") // Always blank as noted in deprecation message
	...
	data.SubGroupEvents = types.BoolValue(hook.SubGroupEvents)
	data.EmojiEvents = types.BoolValue(hook.EmojiEvents)
